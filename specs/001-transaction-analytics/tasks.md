...
- [ ] T001 Create the directory skeleton per plan.md: `src/common/`, `src/datagen/`,
...
- [ ] T002 Create `docker-compose.yml` skeleton at repo root defining `postgres`, `minio`,
      `redpanda`, `airflow-webserver`, `airflow-scheduler`, and `metabase` services with
      healthchecks (no application logic wired in yet)
- [ ] T003 [P] Add Python dependency manifest (`pyproject.toml`) pinning `faker`, `boto3`,
      `confluent-kafka`, `scikit-learn`, `pandas`, `pytest`, `dbt-postgres`
- [ ] T004 [P] Configure `ruff`/`black` for `src/` and `tests/`
...
- [ ] T005 [P] Implement the shared Account/Merchant/Transaction schema (dataclasses or Pydantic
...
- [ ] T006 [P] Add Postgres init script in `infra/postgres/` creating the `finpulse` (warehouse)
      and `airflow` (metadata) databases in the same instance, per research.md
- [ ] T007 [P] Add MinIO startup script in `infra/minio/` that creates the `finpulse-raw` bucket
      used by both the batch and streaming paths
- [ ] T008 Initialize the dbt project skeleton in `dbt/` (`dbt_project.yml`, `profiles.yml`
...
- [ ] T009 Initialize Airflow config in `airflow/` referencing `airflow/dags/` and using the
...
- [ ] T010 [P] [US1] Implement synthetic account/merchant generation (~500 accounts, ~200
      merchants) in `src/datagen/generate.py`, using `src/common/schema.py` — depends on T005
- [ ] T011 [P] [US1] Implement synthetic historical transaction generation (~50,000 transactions,
      `source="batch"`) in `src/datagen/generate.py`, using `src/common/schema.py` — depends on
      T005
- [ ] T012 [US1] Implement the batch loader that writes generated accounts/merchants/transactions
      as newline-delimited JSON into the `finpulse-raw` MinIO bucket — depends on T007, T010, T011
- [ ] T013 [P] [US1] Create dbt staging models (`stg_accounts`, `stg_merchants`,
      `stg_transactions`) in `dbt/models/staging/` reading the MinIO-sourced raw data — depends on
      T008
- [ ] T014 [US1] Create dbt marts models (`fct_transactions`, `dim_account`, `dim_merchant`) in
      `dbt/models/marts/` supporting volume-over-time and revenue-by-merchant/region aggregation —
      depends on T013
- [ ] T015 [US1] Implement `airflow/dags/batch_load_dag.py` orchestrating generate → load to
      MinIO → `dbt run` (staging + marts), scheduled to run on startup — depends on T012, T014
- [ ] T016 [P] [US1] Write version-controlled Metabase question/dashboard definitions (volume over
      time, revenue by merchant/region) as JSON in `infra/metabase/`
- [ ] T017 [US1] Implement `infra/metabase/provision_dashboard.py`, calling the Metabase REST API
      on first boot to create the Postgres connection and the dashboard from T016 — depends on
      T016
- [ ] T018 [US1] Wire `batch_load_dag`, dbt, and Metabase provisioning into `docker-compose.yml`
...
- [ ] T019 [P] [US2] Implement feature extraction (amount, time-of-day, merchant category,
      account velocity) in `src/ml/fraud_scoring/features.py`, reading from `fct_transactions` —
      depends on T014
- [ ] T020 [US2] Implement `IsolationForest` batch scoring in `src/ml/fraud_scoring/score.py`,
      producing `risk_score` + `is_flagged` per transaction and writing to a new
      `fct_fraud_scores` table in Postgres — depends on T019
- [ ] T021 [US2] Add a dbt source/marts definition for `fct_fraud_scores` in
      `dbt/models/marts/` so it's queryable alongside `fct_transactions` — depends on T020
- [ ] T022 [US2] Implement `airflow/dags/fraud_scoring_dag.py` orchestrating `dbt run`
      (incremental) → `src/ml/fraud_scoring/score.py`, scheduled after `batch_load_dag` — depends
      on T015, T020
- [ ] T023 [P] [US2] Add the flagged-transaction-rate question/card to the Metabase definitions in
      `infra/metabase/` — depends on T016, T021
- [ ] T024 [US2] Wire `fraud_scoring_dag` into the `docker-compose.yml` startup sequence —
...
- [ ] T031 [P] Contract tests in `tests/contract/test_transaction_schema.py` validating that both
      `src/datagen` output and `src/streaming/producer` output conform to
      `contracts/transaction-event-schema.md` — depends on T011, T026
- [ ] T032 [P] Unit tests for `src/ml/fraud_scoring/score.py` in
      `tests/unit/test_fraud_scoring.py` — depends on T020
- [ ] T033 Integration/smoke test in `tests/integration/test_end_to_end.py` running through
      `quickstart.md`'s validation steps against a live stack — depends on T018, T024, T030
- [ ] T034 Run a full clean-checkout validation (`docker compose down -v` then
      `docker compose up -d`) and confirm SC-001 through SC-005 all pass — depends on T033
...
2. + US1 → demo the batch pipeline + dashboard (MVP)
3. + US2 → demo fraud-risk scoring on top
4. + US3 → demo live streaming freshness on top
5. + Polish → contract/integration tests, full clean-checkout validation of all 5 success criteria
